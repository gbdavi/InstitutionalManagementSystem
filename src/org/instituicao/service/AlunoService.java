...
public class AlunoService {
    ...
    public AlunoDTO cadastrar(AlunoCadastroDTO alunoCadastroDTO) {
        ...
        AlunoEntity aluno = new AlunoEntity(
                alunoCadastroDTO.getCpf(),
                alunoCadastroDTO.getNome(),
                alunoCadastroDTO.getDataNascimento(),
                alunoCadastroDTO.getEmail(),
                instituicao.get());
    ...
    public RelatorioAlunoDTO getRelatorio(int matriculaAluno) {
        ...
        List<RelatorioCursoDTO> relatorioCursos = aluno.get().getCursos().stream()
            .map(curso -> new RelatorioCursoDTO(
                new CursoDTO(curso),
                curso.getDisciplinas().stream()
                    .map(disciplina -> {
                        DisciplinaDTO disciplinaDTO = new DisciplinaDTO(disciplina);
                        Optional<TurmaEntity> turmaDisciplina = turmasAluno.stream()
                                .filter(turmaEntity -> turmaEntity.getDisciplina() == disciplina)
                                .findFirst();
                        if (turmaDisciplina.isPresent()) {
                            if (turmaDisciplina.get().getStatus() == StatusTurma.CONCLUIDA) {
                                ...
                                SituacaoDisciplina situacaoDisciplina = media >= 7
                            ...
                            SituacaoDisciplina situacaoDisciplina = turmaDisciplina.get().getStatus() == StatusTurma.NAO_INICIADA
                                    ? SituacaoDisciplina.A_CURSAR
